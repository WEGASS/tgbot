...
def get_new_ads(search, message):
    ...
    try:
        ...
        ads_list = []
        for ad in ads:
            title = ad.find_element_by_css_selector(
                'div[class = "iva-item-titleStep-_CxvN"]').find_element_by_css_selector('a').get_attribute('title')
            href = ad.find_element_by_css_selector(
                'div[class = "iva-item-titleStep-_CxvN"]').find_element_by_css_selector('a').get_attribute('href')
            price = ad.find_element_by_css_selector(
                'div[class = "iva-item-priceStep-QN8Kl"]').find_element_by_css_selector(
...
def new_thread(message):
    t = threading.Thread(target=schedule_ads, args=[message])
    t.start()
    print(t)
    send_welcome(message)
